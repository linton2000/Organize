...
export default function LastWorked(props: { lastWorkedDate: string | null }) {
    ...
    useEffect(() => {
        if (!props.lastWorkedDate) return;
    ...
    if (props.lastWorkedDate) {
        const interval: Interval = calcInterval(
            new Date(props.lastWorkedDate),
            new Date(now)
        );

        let dayStr: string =
            String(interval.days) + (interval.days > 1 ? " days" : " day");
        let hrStr: string =
            String(interval.hrs) + (interval.hrs > 1 ? " hrs" : " hr");
        let minStr: string =
            String(interval.mins) + (interval.mins > 1 ? " mins" : " min");
    ...
    return (
        <p style={{ fontFamily: "Georgia, serif" }}>
            Last Worked:{" "}
            <span style={{ color: colours[colour_i] }}>{lastWorkedStr}</span>
        </p>
    );
}
